...
human_template = system_template + """{question}"""
...
def factory():
    ...
    #sys_prompt = SystemMessagePromptTemplate(prompt=prompt)
    human_prompt = PromptTemplate(template=human_template, input_variables=['question'])
    #human_prompt = PromptTemplate.from_template(template=human_template, input_variables=['question'])
    ...
    llm_chain = LLMChain(prompt=human_prompt, llm=OpenAI(temperature=0), verbose=True)
